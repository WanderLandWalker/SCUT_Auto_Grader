...
### 方式一：从脚本平台安装（推荐）
 ...
 [![Greasy Fork](https://img.shields.io/badge/-Greasy_Fork-green?style=flat)](https://greasyfork.org/zh-CN/scripts/580429) [![ScriptCat](https://img.shields.io/badge/-ScriptCat-orange?style=flat)](https://scriptcat.org/zh-CN/script-show-page/6456)
...
- **API Key**：保存在浏览器 `localStorage` 中，仅在本机使用，不会上传到任何服务器。提供"清除已保存的API Key"按钮
- **AI评分数据**：使用AI评分模式时，题目、参考答案、学生回答会发送到用户选择的AI供应商服务器（如DeepSeek、OpenAI等）。脚本首次使用时会弹出隐私提示
- **默认满分/导出模式**：不涉及任何外部数据传输
- **无遥测**：脚本不收集、不上传任何用户数据或使用统计
